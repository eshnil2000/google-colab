...
class TestDanku(TestDankuTestCase):
	...
	def test_2lnn_load_dataset(self):
		il_nn = 2
		hl_nn = []
		ol_nn = 2
		nn = NeuralNetwork(il_nn, hl_nn, ol_nn)
	...
	def test_create_3_layer_nn(self):
		il_nn = 2
		hl_nn = [4]
		ol_nn = 2
		nn = NeuralNetwork(il_nn, hl_nn, ol_nn)
	...
	def test_create_5_layer_nn(self):
		il_nn = 2
		hl_nn = [4,5,6]
		ol_nn = 2
		nn = NeuralNetwork(il_nn, hl_nn, ol_nn)
	...
	def test_5lnn_load_dataset(self):
		il_nn = 2
		hl_nn = [4,5,6]
		ol_nn = 2
		nn = NeuralNetwork(il_nn, hl_nn, ol_nn)
	...
	def test_unpacking_weights(self):
		il_nn = 2
		hl_nn = [2,3]
		ol_nn = 2
		nn = NeuralNetwork(il_nn, hl_nn, ol_nn)
		...
		for l in range(len(unpacked_weights)):
			for n in range(len(unpacked_weights[l])):
				for w in range(len(unpacked_weights[l][n])):
					assert(unpacked_weights[l][n][w] ==expected_unpacked_weights[l][n][w])

	def test_packing_biases(self):
		il_nn = 2
		hl_nn = [2,3]
		ol_nn = 2
		nn = NeuralNetwork(il_nn, hl_nn, ol_nn)
	...
	def test_unpacking_biases(self):
		il_nn = 2
		hl_nn = [2,3]
		ol_nn = 2
		nn = NeuralNetwork(il_nn, hl_nn, ol_nn)
...
suite = unittest.TestLoader().loadTestsFromTestCase(TestDanku)
